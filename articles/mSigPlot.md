...
By default `plot_title` is placed inside the plot area. Set
`title_outside_plot = TRUE` to use a standard ggplot title above the
panel instead. Set `grid = TRUE` to add horizontal reference lines at
the y-axis breaks. These options are independent and are supported by
...
![](mSigPlot_files/figure-html/sbs96-grid-title-outside-1.png)
